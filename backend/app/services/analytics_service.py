...
class AnalyticsService:
    ...
    async def get_analytics(
        self, *, assessment_type: str | None = None
    ) -> AnalyticsResult:
        logger.info(
            build_log_message("analytics_started", assessment_type=assessment_type)
        )
        ...
        raw_normalized = {
            item["_id"]: item["count"]
            for item in raw.get("score_distribution_normalized", [])
        }
        score_distribution_normalized = []
        for i, boundary in enumerate(bucket_boundaries):
            label = bucket_labels[i]
            count = raw_normalized.get(boundary, 0)
            score_distribution_normalized.append(
                ScoreDistributionBucket(bucket=label, count=count)
            )
        # Handle the "100" default bucket
        if "100" in raw_normalized:
            score_distribution_normalized.append(
                ScoreDistributionBucket(bucket="100", count=raw_normalized["100"])
            )
        ...
        level_accuracy = [
            LevelAccuracyStat(
                level=item["_id"], mean_accuracy=item.get("mean_accuracy")
            )
