...
async def get_questions_for_assessment(*, assessment_id: str, db):
    assess_oid = _ensure_object_id(assessment_id)
    assessment = await assessments_collection(db).find_one({"_id": assess_oid})
    if not assessment:
        raise AppError(status_code=404, code="ASSESSMENT_NOT_FOUND", message="Assessment not found")

    question_docs = await questions_collection(db).find({}).to_list(length=None)
    answer_docs = await answers_collection(db).find({"assessment_id": assess_oid}).to_list(length=None)
...
async def upsert_answer(*, assessment_id: str, question_id: str, selected_option: str, db):
    assess_oid = _ensure_object_id(assessment_id)
    question_oid = _ensure_object_id(question_id, code="INVALID_QUESTION_ID")

    question = await questions_collection(db).find_one({"_id": question_oid})
    if not question:
    ...
    await answers_collection(db).update_one(
...
async def submit_assessment(*, assessment_id: str, db):
    assess_oid = _ensure_object_id(assessment_id)
    assessment = await assessments_collection(db).find_one({"_id": assess_oid})
    if not assessment:
        raise AppError(status_code=404, code="ASSESSMENT_NOT_FOUND", message="Assessment not found")
    ...
    questions = await questions_collection(db).find({}, {"_id": 1}).to_list(length=None)
    question_ids = {str(item["_id"]) for item in questions}

    answers = await answers_collection(db).find({"assessment_id": assess_oid}, {"question_id": 1}).to_list(length=None)
    ...
    result = await assessments_collection(db).find_one_and_update(
    ...
    if result is None:
        latest = await assessments_collection(db).find_one({"_id": assess_oid})
